...
class TemporalBlock(BaseTCN):
    ...
    def apply_film(self, x, gamma, beta):
        ...
        # gamma/beta: [batch_size, channels]
    ...
    def forward(
            ...
            inference,
            embeddings = None,
            theta: Optional[torch.Tensor] = None,  # NEW: Static parameters
            film_params=None,  # Changed from embeddings to film_params
            in_buffers=None,
            ):
        ...
        # Apply FiLM conditioning if parameters provided
...
class TCN(BaseTCN):
    def __init__(
            ...
            static_dim: int = None,  # NEW: Dimension of static parameters
            kernel_size: int = 4,
            ...
            film_hidden_dims: list = [64, 128],  # NEW: FiLM generator dimensions
            ):
        ...
        if static_dim is not None:
            if use_gate:
                # For gated blocks, channel size doubles after first conv
                film_channel_sizes = [2 * c for c in num_channels]
            else:
                film_channel_sizes = num_channels
                
            self.film_generator = FiLMGenerator(
                static_dim=static_dim,
                hidden_dims=film_hidden_dims,
                num_blocks=len(num_channels),
                channel_sizes=film_channel_sizes
            )
        else:
    ...
    def forward(
        ...
        # NEW: Generate FiLM parameters if static params provided
        film_params = self.film_generator(theta) if self.film_generator is not None and theta is not None else None
        # film_params = None
        # if self.film_generator is not None and theta is not None:
        #     film_params = self.film_generator(theta)  # List of (gamma, beta) tuples
        ...
        if self.use_skip_connections:
            ...
            for index, layer in enumerate( self.network ):
                ...
                # NEW: Pass film parameters for this block
                block_film = film_params[index] if film_params else None
                x, skip_out = layer(
                    x,
                    film_params=block_film,  # Pass FiLM params instead of embeddings
                    embeddings=embeddings,
                    inference=inference,
