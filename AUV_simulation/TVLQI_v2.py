...
class OneStepTVLQIController:
    ...
    def _compute_gains(self, A_aug, B_aug, Q_aug, R_aug, k_time):
        ...
        try:
            ...
            # Fallback to Riccati iteration
            try:
                # Initialize with Q matrix
                P = Q_aug.copy()
                
                # Precompute for efficiency
                A_aug_T = A_aug.T
                
                for i in range(20):  # Reduced iteration count
                    # Regularized solve
                    S = R_aug + B_aug.T @ P @ B_aug + 1e-8 * np.eye(self.nu)
                    K_pi = np.linalg.solve(S, B_aug.T @ P @ A_aug)
                    
                    # Efficient Riccati update
                    A_minus_BK = A_aug - B_aug @ K_pi
                    P_next = A_aug_T @ P @ A_minus_BK + Q_aug
                    
                    # Check convergence (relative change)
                    delta = np.max(np.abs(P_next - P))
                    if delta < 1e-8 * np.max(np.abs(P)):
                        break
                        
                    P = P_next
                    
                # Final gain calculation
                S_final = R_aug + B_aug.T @ P @ B_aug
                K_aug = np.linalg.solve(S_final, B_aug.T @ P @ A_aug)
                method = "riccati"
                fallback = False
                
            except Exception as riccati_err:
                # Final fallback to one-step
